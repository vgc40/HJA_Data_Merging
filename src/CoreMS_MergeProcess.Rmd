...
# Change directory
#setwd(paste0(path_to_dir,"/CoreMS_Output"))

# list CoreMS output files
file.list = list.files(pattern = "corems.csv", path = paste0(path_to_dir,"/CoreMS_Output"), full.names = T)
...
for(file in file.list){
  ...
  # storing sample name
  samp.name <- sub(".*(HJ.*)\\.corems\\.csv$", "\\1", file)
...
peak_icr = ftmsRanalysis::as.peakData(e_data = as.data.frame(data), f_data = factor, e_meta = as.data.frame(mol), 
